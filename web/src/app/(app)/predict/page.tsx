...
export default function PredictWizard() {
  ...
  // Exam year is auto-selected (profile year, else latest cycle) and stays editable.
  ...
  // Recompute the estimated AIR from the current score + exam year.
  ...
  useEffect(() => {
    fetch("/api/auth/me").then((r) => r.json()).then(({ user }) => {
      if (!user) return;
      // If the student saved a score but no rank, open in score mode. Prefer the
      // student's own saved exam year; the state default (latest) applies otherwise.
